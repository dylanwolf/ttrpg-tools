			...
			<>
				...
				<div className="available">
					{stepState.LowLevelChoices > 0 ? (
						<div className="level">
							Low Level remaining: {stepState.LowLevelChoices}
						</div>
					) : (
						<></>
					)}
					{stepState.MidLevelChoices > 0 ? (
						<div className="level">
							Mid Level remaining: {stepState.MidLevelChoices}
						</div>
					) : (
						<></>
					)}
					{stepState.HighLevelChoices > 0 ? (
						<div className="level">
							High Level remaining: {stepState.HighLevelChoices}
						</div>
					) : (
						<></>
					)}
					...
					<tbody className="table-group-divider">
						{stepState.SelectList.map((spell) => (
							<tr key={`RyuutamaSpellSelector-${this.Name}-${spell.Name}`}>
								...
								<td>{spell.DataSource || "Ryuutama"}</td>
							</tr>
						))}
					</tbody>
				</table>
